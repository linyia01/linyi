...
class Net(object):
    ...
    def __init__(self, mode=Mode.TRAIN, debug=False):
        ...
        self.mode = mode
        self.debug = debug
        self.cam = cv2.VideoCapture('D:/ac.mp4')
    ...
    def test(self, checkpoint, input_a_path, input_b_path, out_path, save_image=True, save_flo=False):
        ...
        # }
        xp = tf.placeholder(tf.float32, [1, 384, 512, 3])
        xn = tf.placeholder(tf.float32, [1, 384, 512, 3])
        input = {
            'input_a': xp,
            'input_b': xn,
        }
        predictions = self.model(input, training_schedule)
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                img0 = cv2.cvtColor(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2RGB)
                img0 = cv2.resize(img0, (512, 384))
                ret, frame = self.cam.read()
                ret, frame = self.cam.read()
                ret, frame = self.cam.read()
                img1 = cv2.cvtColor(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2RGB)
                ...
                pred_ = sess.run(pred_flow, feed_dict={xp: np.reshape(input_a, (1, 384, 512, 3)),
                                                           xn: np.reshape(input_b, (1, 384, 512, 3)), }
                                     )[0, :, :, :]
                unique_name = 'flow_'+str(index)+'_' + str(uuid.uuid4())[0:3]
                if save_image:
                    flow_img = flow_to_image(pred_)
                    full_out_path = os.path.join(out_path, unique_name + '.png')
                    img = Image.fromarray(img0)
                    img.save(os.path.join(out_path, unique_name + 'b.png'))
                    img = Image.fromarray(img1)
                    img.save(os.path.join(out_path, unique_name + 'e.png'))
                    flow_img = cv2.cvtColor(cv2.cvtColor(flow_img, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2RGB)
                    imsave(full_out_path, flow_img)

                if save_flo:
                    full_out_path = os.path.join(out_path, unique_name + 'linyi.flo')
                    write_flow(pred_, full_out_path)
