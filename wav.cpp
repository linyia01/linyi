...
double * wavFile_execute(const char * fileName, WAV_STRUCT * wavFileEg){
	FILE * fin = NULL;
...
short * wavFile_execute_short(const char * fileName, WAV_STRUCT * wavFileEg){
	FILE * fin = NULL;
	fin = fopen(fileName,"rb");
	if(!fin){
		printf("%s cannot open this file\n", fileName);
	...
	short *originData = (short *)malloc(sizeof(short)*wavFileEg->numSamples);
	int gap = wavFileEg->wavHeader.fmt.bitsPerSample / 8 * wavFileEg->wavHeader.fmt.numChannels;
	if(wavFileEg->wavHeader.fmt.bitsPerSample == 8){
		i = 0;
		while(i < wavFileEg->numSamples){
		...
		i = 0;
		while(i < wavFileEg->numSamples){
			short tmp = (wavFileEg->data[i*gap+1])<<8|(wavFileEg->data[i*gap]);
			originData[i]=tmp;
			++i;
		}
	}
